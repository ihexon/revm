...
build() {
    ...
    if [[ "$os" == "Darwin" ]]; then
        ...
        # Rewrite Homebrew absolute paths in libkrun / libkrunfw dylibs
        for lib in "$bindir"/libkrun*.dylib; do
            for dylib in "${HOMEBREW_DYLIBS[@]}"; do
                install_name_tool -change \
                    "$HOMEBREW_PREFIX/opt/$dylib" \
                    "@loader_path/$(basename "$dylib")" \
                    "$lib" 2>/dev/null || true
            done
        done

        # Fix virglrenderer's own dependencies
        install_name_tool -change \
            "$HOMEBREW_PREFIX/opt/libepoxy/lib/libepoxy.0.dylib" \
            "@loader_path/libepoxy.0.dylib" \
            "$bindir/libvirglrenderer.1.dylib" 2>/dev/null || true
        install_name_tool -change \
        ...
        for dylib in "${HOMEBREW_DYLIBS[@]}"; do
            local lib="$bindir/$(basename "$dylib")"
