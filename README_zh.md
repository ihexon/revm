...
Linux Rootfs 环境中快速执行命令）和 **dockerd 模式**（通过 `revm dockerd` 子命令在 Apple Silicon/Linux 上运行完整的 Podman 容器引擎）。
...
| [dockerd 模式](docs/docker-mode_zh.md) | 无需 Docker Desktop 的完整容器引擎——兼容 Podman/Docker CLI |
