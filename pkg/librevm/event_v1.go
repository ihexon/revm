...
func (vm *VM) registerV1EventSink() {
	endpoint := vm.cfg.V1EventReportURL
	...
	if runMode == "" {
		logrus.Warnf("v1 event sink: run mode is empty, skip reporting to %q", endpoint)
		return
	}
	...
	vm.eventDispatcher.addHandler(SinkV1, handler, closer)
}
